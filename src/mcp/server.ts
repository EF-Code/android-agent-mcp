...
function verificationData(observation: ActionObservation): Record<string, unknown> {
  ...
  const foregroundChanged =
    observation.before !== null && observation.after !== null
      ? observation.before.foreground.packageName !== observation.after.foreground.packageName ||
        observation.before.foreground.activity !== observation.after.foreground.activity
      : null;
  const pixelChanged =
...
function registerReadOnlyTools(server: McpServer, service: AndroidDeviceService): void {
  registerTool(
    ...
    'device_list',
    {
      description: 'List connected Android devices and their ADB authorization states.',
  ...
  registerTool(
    ...
    'device_info',
    {
      description: 'Return normalized information for the selected device.',
      inputSchema: emptySchema,
    },
    async () => {
      try {
  ...
  registerTool(
    ...
    'mirror_status',
    {
      description: 'Return the status of the server-owned scrcpy process.',
      inputSchema: emptySchema,
    },
    async () => {
      try {
        return jsonContent(ok(service.scrcpy.status()));
      } catch (error) {
        return toolError(error);
      }
    },
  ...
  registerTool(
    ...
    'screen_capture',
    {
      description: 'Capture a validated PNG screenshot from the selected device.',
  ...
  registerTool(
    ...
    'ui_dump',
    {
      description: 'Capture and normalize the selected device UIAutomator hierarchy.',
      inputSchema: uiDumpSchema,
    },
    async () => {
      try {
        ...
        if (evidence !== null && !evidence.paused)
          await evidence.saveUi(`ui-dump-${Date.now()}`, snapshot);
        const data = {
          ...snapshot,
          nodes: snapshot.nodes.map((node) => ({
            ...node,
            ...(node.flags.password ? { text: '[REDACTED]' } : {}),
          })),
        };
        return jsonContent(
          ok(data, { deviceSerial: await service.selectedSerial(), warnings: snapshot.warnings }),
        );
      } catch (error) {
        return toolError(error);
      }
  ...
  registerTool(
    ...
    'ui_find',
    {
      description: 'Find matching elements in a fresh or retained UI snapshot.',
      inputSchema: uiFindSchema,
    },
    async (args) => {
      try {
        const snapshot =
          args.snapshot_id === undefined
            ? await service.captureUi()
            : await service.requireFreshSnapshot(args.snapshot_id);
        const matches = (await import('../ui/selectors.js')).findMatches(
          snapshot,
          toSelector(args.selector),
        );
        return jsonContent(
          ok(
            { snapshot_id: snapshot.snapshotId, matches },
            { deviceSerial: await service.selectedSerial(), warnings: snapshot.warnings },
          ),
        );
      } catch (error) {
        return toolError(error);
  ...
  registerTool(
    ...
    'app_list',
    {
      description: 'List bounded package metadata from the selected device.',
    ...
    async (args) => {
      try {
        ...
        return jsonContent(
          ok(await service.packages.list(serial, options), { deviceSerial: serial }),
        );
      } catch (error) {
        return toolError(error);
      }
  ...
  registerTool(
    ...
    async (args) => {
      try {
        ...
        return jsonContent(
          ok(await service.packages.info(await service.selectedSerial(), args.package_name), {
            deviceSerial: await service.selectedSerial(),
          }),
        );
      } catch (error) {
  ...
  registerTool(
    ...
    async (args) => {
      try {
        service.policy.assertPackageAllowed(args.package_name);
        return jsonContent(
          ok(await service.permissions.list(await service.selectedSerial(), args.package_name), {
            deviceSerial: await service.selectedSerial(),
          }),
        );
      } catch (error) {
  ...
  registerTool(
    ...
    'logcat_capture',
    {
      description: 'Capture bounded, filtered, and redacted logcat output.',
      inputSchema: logCaptureSchema,
    },
    async (args) => {
      try {
        ...
        if (args.package_name === undefined && args.pid === undefined) {
          throw new AppError(
            ...
            'Log capture requires a package name or PID filter.',
          );
        }
        ...
        const options = {
          ...
          ...(args.include_crash_buffer === undefined
            ? {}
  ...
  registerTool(
    ...
    async (args) => {
      try {
        service.policy.assertPackageAllowed(args.package_name);
        return jsonContent(
          ok(await service.logcat.crashes(await service.selectedSerial(), args.package_name), {
            deviceSerial: await service.selectedSerial(),
          }),
        );
      } catch (error) {
...
function registerInteractiveTools(server: McpServer, service: AndroidDeviceService): void {
  registerTool(
    ...
    'device_select',
    {
      description: 'Select one authorized Android device by exact serial.',
      inputSchema: serialSchema,
    },
    async (args) => {
      try {
  ...
  registerTool(
    ...
    'mirror_start',
    {
      description: 'Start a visible server-owned scrcpy mirror for the selected device.',
      inputSchema: mirrorStartSchema,
    },
    async (args) => {
      try {
        const serial = await service.selectedSerial();
  ...
  registerTool(
    ...
    service,
    'mirror_stop',
    { description: 'Stop only the server-owned scrcpy mirror.', inputSchema: emptySchema },
    async () => {
      try {
        return jsonContent(ok(await service.scrcpy.stop()));
      } catch (error) {
        return toolError(error);
      }
    },
  ...
  registerTool(
    ...
    'ui_tap',
    {
      description: 'Tap one uniquely resolved visible semantic UI element and verify the result.',
      inputSchema: uiTapSchema,
    },
    async (args) => {
      try {
        const selectorNodeId = (args.selector as { nodeId?: string } | undefined)?.nodeId;
        const requestedNodeId = args.node_id ?? selectorNodeId;
        if (args.selector === undefined && requestedNodeId === undefined)
          throw new AppError(ErrorCode.InvalidInput, 'selector or node_id is required');
        if (requestedNodeId !== undefined && args.snapshot_id === undefined)
          throw new AppError(
            ErrorCode.InvalidInput,
            'snapshot_id is required when ui_tap uses a snapshot-local node_id.',
          );
        const selector =
          args.selector === undefined ? { nodeId: requestedNodeId! } : toSelector(args.selector);
        const sourceSnapshot =
          args.snapshot_id === undefined
            ? undefined
            : await service.requireFreshSnapshot(args.snapshot_id);
        const result = await service.tapSelector(
          selector,
          args.match_index,
        ...
        return jsonContent(
          ok(
            { node_id: result.nodeId, ...verificationData(result) },
            {
              deviceSerial: await service.selectedSerial(),
  ...
  registerTool(
    ...
    'screen_tap',
    {
      description: 'Tap validated native device-pixel coordinates as a fallback.',
  ...
  registerTool(
    ...
    'screen_swipe',
    {
      description: 'Perform a bounded native coordinate or deterministic directional swipe.',
      inputSchema: swipeSchema,
    },
    async (args) => {
      try {
        const serial = await service.selectedSerial({ checkConnection: false });
    ...
    async (args) => {
      try {
        ...
        return visualFrameContent(result, {
          ...
          frame_transport: result.frameTransport,
  ...
  registerTool(
    ...
    'screen_long_press',
    {
      description: 'Perform a bounded stationary long press.',
      inputSchema: {
        ...coordinateSchema,
        duration_ms: z.number().int().min(250).max(30_000).optional(),
      },
    },
    async (args) => {
      try {
  ...
  registerTool(
    ...
    'text_type',
    {
      ...
      inputSchema: textTypeSchema,
    },
    async (args) => {
      try {
        await service.requireAllowedForeground('text entry');
        const snapshot = await service.captureUi();
        if (snapshot.nodes.some((node) => node.flags.focused && node.flags.password))
          throw new AppError(
  ...
  registerTool(
    ...
    async (args) => {
      try {
        ...
        const crashes = await service.logcat.crashes(
          serial,
          args.package_name,
          100,
          crashBaselineEpoch,
          ...
          ok(
            {
              ...
              foreground,
              immediate_crashes: crashes,
              crash_evidence_since_epoch: crashBaselineEpoch,
            },
            { deviceSerial: serial },
          ),
        );
  ...
  registerTool(
    ...
    async (args) => {
      try {
        service.policy.assertPackageAllowed(args.package_name);
        const serial = await service.selectedSerial();
        await service.packages.stop(serial, args.package_name);
        return jsonContent(
          ok({ package_name: args.package_name, stopped: true }, { deviceSerial: serial }),
        );
      } catch (error) {
        return toolError(error);
      }
  ...
  registerTool(
    ...
    'wait_for_ui',
    {
      description: 'Poll for a bounded UI, foreground, disappearance, or screen-change condition.',
      inputSchema: waitForUiSchema,
    },
    async (args) => {
      try {
        if (args.package_name !== undefined) service.policy.assertPackageAllowed(args.package_name);
        ...
        return jsonContent(
          ok(await service.waitForUi(options), { deviceSerial: await service.selectedSerial() }),
        );
      } catch (error) {
        return toolError(error);
      }
...
function registerMutationTools(server: McpServer, service: AndroidDeviceService): void {
  registerTool(
    ...
    'app_install',
    {
      description: 'Approval-required installation of an APK under an allowed host root.',
      inputSchema: installSchema,
    },
    async (args) => {
      try {
        service.policy.assertMutationAllowed('app_install');
        const serial = await service.selectedSerial();
        const apk = await service.installer.validate(args.path);
        return jsonContent(
          ok(await service.installer.install(serial, apk, args.replace ?? false), {
            deviceSerial: serial,
          }),
        );
      } catch (error) {
  ...
  registerTool(
    ...
    async (args) => {
      try {
        service.policy.assertMutationAllowed('app_clear_data');
        service.policy.assertPackageAllowed(args.package_name);
        const serial = await service.selectedSerial();
        const output = await service.packages.clearData(serial, args.package_name);
        return jsonContent(
          ok(
            { package_name: args.package_name, output, irreversible: true },
            { deviceSerial: serial },
          ),
        );
      } catch (error) {
  ...
  registerTool(
    ...
    'permissions_set',
    {
      description: 'Approval-required grant or revoke of a requested runtime permission.',
      inputSchema: permissionSetSchema,
    },
    async (args) => {
      try {
        service.policy.assertMutationAllowed('permissions_set');
        service.policy.assertPackageAllowed(args.package_name);
        const serial = await service.selectedSerial();
        await service.permissions.set(serial, args.package_name, args.permission, args.action);
        return jsonContent(
          ok(
            { package_name: args.package_name, permission: args.permission, action: args.action },
            { deviceSerial: serial },
          ),
        );
      } catch (error) {
...
function registerEvidenceTools(server: McpServer, service: AndroidDeviceService): void {
  registerTool(
    ...
    'evidence_begin',
    {
      description: 'Begin a sanitized, bounded local evidence session.',
      inputSchema: evidenceBeginSchema,
    },
    async (args) => {
      try {
        const session = await service.beginEvidence(args.label, args.metadata);
        return jsonContent(ok(session.summary));
      } catch (error) {
        return toolError(error);
      }
    },
  ...
  registerTool(
    ...
    'evidence_note',
    {
      description: 'Add a redacted note to the active evidence session.',
      inputSchema: evidenceNoteSchema,
    },
    async (args) => {
      try {
        const session: EvidenceSession = service.evidence.requireActive();
        await session.note(args.message, args.details);
        return jsonContent(ok({ evidence_id: session.evidenceId }));
      } catch (error) {
        return toolError(error);
      }
    },
  ...
  registerTool(
    ...
    'evidence_finish',
    {
      description: 'Finish the active evidence session and write its summary.',
      inputSchema: evidenceFinishSchema,
    },
    async () => {
      try {
        return jsonContent(ok(await service.evidence.finish()));
      } catch (error) {
        return toolError(error);
      }
    },
...
export function createMcpServer(config: ServerConfig = loadConfig()): {
  server: McpServer;
  service: AndroidDeviceService;
} {
  const service = new AndroidDeviceService(config);
  const server = new McpServer(
    { name: 'android-device', version: '0.5.1' },
