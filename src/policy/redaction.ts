...
const LOG_PATTERNS: Array<[RegExp, string]> = [
  ...
  [/(set-cookie\s*[:=]\s*)[^\r\n]+/gi, `$1${REDACTED}`],
  [
    /(\b(?:token|secret|password|passwd|api[_-]?key|access[_-]?key)\s*[:=]\s*)[^\s,;]+/gi,
    `$1${REDACTED}`,
  ],
  [
    /(\b[A-Za-z0-9_-]*(?:token|secret|password|passwd|api[_-]?key)[A-Za-z0-9_-]*\s+)[^\s,;]+/gi,
    `$1${REDACTED}`,
  ],
...
export function redactCommandArgs(
  args: string[],
  secretIndexes: ReadonlySet<number> = new Set(),
): string[] {
