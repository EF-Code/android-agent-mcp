...
test('stops only the owned scrcpy child and preserves an unrelated process', async () => {
  ...
  const runner = {
    run: async () => ({
      ...
      record: { executable: fixture, args: ['--version'], exitCode: 0, signal: null, durationMs: 1, stdoutBytes: 12, stderrBytes: 0, stdoutTruncated: false, stderrTruncated: false },
    }),
  };
  const manager = new ScrcpyProcessManager(fixture, false, runner);
  const unrelated = (await import('node:child_process')).spawn(process.execPath, ['-e', 'setInterval(() => undefined, 1000)'], { detached: true, stdio: 'ignore' });
...
test('marks an owned mirror detached without changing its bound serial', async () => {
  ...
  await chmod(fixture, 0o755);
  const runner = {
    run: async () => ({ stdout: Buffer.from('scrcpy 4.1\n'), stderr: Buffer.alloc(0), record: { executable: fixture, args: [], exitCode: 0, signal: null, durationMs: 1, stdoutBytes: 12, stderrBytes: 0, stdoutTruncated: false, stderrTruncated: false } }),
  };
  const manager = new ScrcpyProcessManager(fixture, false, runner);
  await manager.start('serial-2', { maxSize: 1_600, maxFps: 30, audio: false, control: false, stayAwake: false, turnScreenOff: false, windowTitle: 'test' });
