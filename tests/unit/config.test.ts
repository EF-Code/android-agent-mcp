...
test('loads defaults and applies validated environment overrides', () => {
  const config = loadConfig({ env: { ANDROID_DEVICE_MCP_ALLOWED_PACKAGES: 'com.example.app, com.example.test', ANDROID_DEVICE_MCP_DEFAULT_TIMEOUT_MS: '5000' } });
...
test('applies file config before environment overrides', async () => {
  const root = await mkdtemp(join('/tmp', 'android-device-config-'));
  const configPath = join(root, 'config.json');
  await writeFile(configPath, JSON.stringify({ allowedPackages: ['com.file.app'], defaultTimeoutMs: 8_000 }));
  const config = loadConfig({ configPath, env: { ANDROID_DEVICE_MCP_ALLOWED_PACKAGES: 'com.env.app' } });
