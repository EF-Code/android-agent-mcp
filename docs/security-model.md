# Android Agent MCP security model quick reference
...
| Interactive/reversible     | Semantic tap, coordinate input, swipe, long press, configured-package launch/stop, safe text, owned scrcpy start/stop     | Requires selected device and policy checks                                       |
...
The default package policy is `*`, so the agent can control installed applications, including Android Settings, without per-app setup. Banking, wallet, and password-style package patterns remain fail-closed by default; set an explicit narrower `allowedPackages` list for a test-only deployment.
