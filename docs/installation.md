...
```zsh
npx -y android-agent-mcp@0.2.0 setup --dry-run
```

Configure one host explicitly:

```zsh
npx -y android-agent-mcp@0.2.0 setup --client openclaw
```

Print a portable JSON configuration for a host not included in the automatic integrations:

```zsh
npx -y android-agent-mcp@0.2.0 setup --client generic
```

Pass a project-specific Android Agent MCP configuration file to the generated host entry:

```zsh
npx -y android-agent-mcp@0.2.0 setup --config /absolute/path/android-agent-mcp.json
```
