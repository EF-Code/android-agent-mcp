...
The physical suite requires `ANDROID_AGENT_MCP_PHYSICAL=1`, a connected authorized device, an explicit test package, and a repeatable harmless selector. It tests the actual MCP stdio protocol. Normal MCP operation does not require `ANDROID_AGENT_MCP_TEST_PACKAGE`; that variable belongs only to the physical fixture.
